...
//!
//! `main.rs` is handled separately: it is allowed to contain EXACTLY fourteen
...
//! create success, create error, and headless plan-contract failure, all pre-`Tui::init()`), the config-load failure (pre-`Tui::init()`),
...
//! The test pins the count to fourteen so a NEW in-frame `eprintln!` added to `main.rs`
...
fn main_rs_has_exactly_the_exempt_print_sites() {
    ...
    assert_eq!(
        ...
        "main.rs must contain EXACTLY the {MAIN_RS_EXEMPT_PRINT_COUNT} fatal, \
         frame-exempt eprintln!/println! sites (CLI dispatch prints for --help/--version/--doctor/unknown/create-success/create-error/plan-contract-error, \
