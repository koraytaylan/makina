...
| 0046 | Front Door — Makina's First Five Minutes for an Outside Developer | 📋 Planned | 0/7 | An outside developer's first five minutes are guided, not hostile: an empty-backend start routes to the Doctor `w` scaffold instead of `exit(1)`, `makina --help/--version/--doctor` behave as a real CLI, the shipped concurrency is a safe 2, a committed vhs demo shows the TUI, and the README matches shipped reality — all gates green. | [status](0046-Front-Door-First-Five-Minutes/STATUS.md) |
