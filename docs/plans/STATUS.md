...
| 0044 | Agent Coverage & Compatibility — Verified ACP Agent Support | 📋 Planned | 0/5 | Makina's ACP agent support is visible and verified: string-id agent requests are answered (no hung turns), the registry lists ten correctly-launched ACP agents, every registry-derived surface stays in sync, and the README publishes a compatibility matrix — all gates green. | [status](0044-Agent-Coverage-And-Compatibility/STATUS.md) |
