...
| `@octokit/core`                  | Tiny (~10 KB) request layer with retry + rate-limit awareness. Composes with the auth strategy above.                               | The full `octokit` meta-package pulls in REST + GraphQL plugins we mostly do not need; using `core` alone is leaner. We add `@octokit/graphql` later **only if** review-thread resolution requires it. |
